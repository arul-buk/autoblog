...
export function normalizeContent(content, config) {
  ...
  // 2. Normalize category to match valid cluster names from config
  ...
  if (categoryMatch) {
    ...
    if (validCategories.length > 0) {
      ...
      if (exact) {
        ...
        // Try partial match (shared significant words)
        ...
        const partial = validCategories.find((c) => {
          ...
          return rawWords.some((w) => cWords.some((cw) =>
            ...
            cw.slice(0, 5) === w.slice(0, 5) // stem match (first 5 chars)
          ));
        });
        if (partial) {
          resolved = partial;
          ...
          resolved = validCategories[0];
        }
      }
    }

    // Slugify the resolved category
