...
import { researchKeywords } from './keyword-research.mjs';
...
function countActiveSteps(config) {
  ...
  if (config.steps.keywordResearch && config.seo?.enabled) count++;
...
export async function runPipeline(config, options = {}) {
  ...
  if (scheduleResult.skipDedupe) {
    ...
    // Add batch-mode additional slugs
    const allMeta = [
      ...existingMeta,
  ...
  if (config.steps.keywordResearch && config.seo?.enabled) {
    log(`${stepLabel()}: Keyword research (DataForSEO)...`);
  ...
  const existingSlugs = getExistingPostMeta(config).map((m) => m.slug);
  let { slug: postSlug, content, metadata } = await writePost(
    apiKey, config, selectedTopic,
    [...existingSlugs, ...additionalSlugs],
    { keywordData, writerNotes: scheduleResult.writerNotes, linkedSlugs }
  );
  ...
  if (config.steps.humanize) {
    ...
    content = await humanizePost(apiKey, config, content);
