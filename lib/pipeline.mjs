 ...
 * 17. Notify — Telegram notification (optional)
 *
...
import { sendNotifications, sendFailureNotification } from './notifications.mjs';
...
export async function runPipeline(config, options = {}) {
  ...
  // embedSchema: true — inject inline JSON-LD <script> blocks into body
  // embedSchema: 'frontmatter-only' — skip inline scripts (site layout handles schema)
  if (config.steps.embedSchema && config.steps.embedSchema !== 'frontmatter-only' && config.output?.siteUrl) {
    try {
  ...
  } else if (config.steps.embedSchema === 'frontmatter-only') {
