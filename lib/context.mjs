...
/**
 * Load credentials from file path or inline JSON.
 * Supports both service account keys and OAuth user credentials.
 */
function loadCredentials(envVar) {
...
/**
 * Get access token from OAuth user credentials.
 */
async function getOAuthAccessToken(credentials) {
  const response = await fetch(TOKEN_URL, {
    method: 'POST',
    headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
  ...
  if (!response.ok) {
    throw new Error(`GA4 OAuth refresh failed (${response.status}): ${await response.text()}`);
  }
...
/**
 * Get access token from either service account or OAuth credentials.
 */
async function getAccessTokenFromCredentials(credentials, scope) {
...
export function loadContext(config) {
  ...
  const empty = { version: 1, posts: [], topicHistory: [], topicBacklog: [], lastRun: null };

  if (!existsSync(filePath)) return empty;

  try {
    const ctx = JSON.parse(readFileSync(filePath, 'utf-8'));
    if (!ctx.topicBacklog) ctx.topicBacklog = [];
    return ctx;
  } catch {
    return empty;
  }
...
// ═══════════════════════════════════════════════════════════════════════════
// Topic Backlog — store unused research topics for future runs
// ═══════════════════════════════════════════════════════════════════════════

const BACKLOG_MAX_AGE_DAYS = 30;
const BACKLOG_MAX_SIZE = 30;
const TIME_SENSITIVE_THRESHOLD = 0.8;

/**
 * Determine if a topic is time-sensitive based on relevance score.
 * Time-sensitive topics (breaking news, trending) should be written immediately.
 * Non-time-sensitive topics can be backlogged for future runs.
 *
 * @param {object} topic - Topic object with relevanceScore
 * @returns {boolean}
 */
export function isTimeSensitive(topic) {
  return (topic.relevanceScore || 0) >= TIME_SENSITIVE_THRESHOLD;
}

/**
 * Save unused topics to the backlog in context.
 * Only saves non-time-sensitive, non-duplicate topics.
 *
 * @param {object} config - Full autoblog config
 * @param {object[]} unusedTopics - Topics that were not selected this run
 * @param {string} selectedSlug - The slug of the topic that was selected (to exclude)
 */
export function saveToBacklog(config, unusedTopics, selectedSlug) {
  const filePath = resolve(process.cwd(), config.context?.filePath || '.autoblog-context.json');
  const ctx = loadContext(config);
  const today = new Date().toISOString().slice(0, 10);

  // Existing backlog slugs for dedup
  const existingSlugs = new Set(ctx.topicBacklog.map((t) => t.title.toLowerCase()));

  for (const topic of unusedTopics) {
    // Skip the selected topic
    if (topic.title.toLowerCase() === selectedSlug?.toLowerCase()) continue;
    // Skip time-sensitive topics — they expire fast
    if (isTimeSensitive(topic)) continue;
    // Skip duplicates already in backlog
    if (existingSlugs.has(topic.title.toLowerCase())) continue;

    ctx.topicBacklog.push({
      title: topic.title,
      ...
      contentFormat: topic.contentFormat || null,
      addedDate: today,
      source: topic.source || 'research',
    });
  }

  // Expire old topics (> 30 days)
  const cutoffDate = new Date(Date.now() - BACKLOG_MAX_AGE_DAYS * 86400000).toISOString().slice(0, 10);
  ctx.topicBacklog = ctx.topicBacklog.filter((t) => t.addedDate >= cutoffDate);

  // Cap backlog size
  if (ctx.topicBacklog.length > BACKLOG_MAX_SIZE) {
    ctx.topicBacklog = ctx.topicBacklog.slice(-BACKLOG_MAX_SIZE);
  }
...
/**
 * Pull topics from the backlog for the current run.
 * Returns backlog topics sorted by relevance, removes them from the backlog.
 *
 * @param {object} config - Full autoblog config
 * @returns {object[]} Topics from backlog (may be empty)
 */
export function pullFromBacklog(config) {
  const ctx = loadContext(config);
  if (!ctx.topicBacklog || ctx.topicBacklog.length === 0) return [];

  // Expire old topics
  const cutoffDate = new Date(Date.now() - BACKLOG_MAX_AGE_DAYS * 86400000).toISOString().slice(0, 10);
  const valid = ctx.topicBacklog.filter((t) => t.addedDate >= cutoffDate);

  // Sort by relevance (highest first)
  valid.sort((a, b) => (b.relevanceScore || 0) - (a.relevanceScore || 0));

  return valid;
}

/**
 * Remove a topic from the backlog after it's been used.
 *
 * @param {object} config - Full autoblog config
 * @param {string} topicTitle - Title of the topic to remove
 */
export function removeFromBacklog(config, topicTitle) {
  const filePath = resolve(process.cwd(), config.context?.filePath || '.autoblog-context.json');
  const ctx = loadContext(config);

  ctx.topicBacklog = (ctx.topicBacklog || []).filter(
...
export async function fetchAnalyticsPerformance(config, slugs) {
  ...
  const credentials = loadCredentials('GA4_SERVICE_ACCOUNT_JSON');
  if (!credentials) return results;

  const propertyId = config.analytics?.propertyId;
  if (!propertyId) return results;

  const accessToken = await withRetry(
    () => getAccessTokenFromCredentials(credentials, GA4_SCOPE),
    { maxAttempts: 2, baseDelayMs: 2000, label: 'ga4-auth' }
  );

  const quotaProject = config.analytics?.quotaProject || config.gsc?.quotaProject || credentials.quota_project_id || null;
      ...
      body: JSON.stringify({
        ...
        dimensionFilter: {
          filter: {
            ...
            stringFilter: {
              matchType: 'CONTAINS',
              value: config.output?.contentPathPrefix || '/blog/',
            },
          },
        },
        limit: 500,
