...
export async function performanceAuditStep(state, config, _options) {
  ...
  // Send audit notification via Telegram (if configured)
  try {
    const { sendAuditNotification } = await import('../notifications.mjs');
    await sendAuditNotification(auditReport, state.geoMetrics, config);
  } catch (err) {
    log(`  Warning: Audit notification failed (${err.message})`);
  }
