...
export async function writeStep(state, config, options) {
  ...
  if (options.costTracker && content) {
    const inputEstimate = 2000;
    const outputEstimate = Math.round(content.length / 4);
    options.costTracker.addGemini('write', { promptTokenCount: inputEstimate, candidatesTokenCount: outputEstimate, totalTokenCount: inputEstimate + outputEstimate }, config.models?.text || 'gemini-3-flash-preview');
  }
