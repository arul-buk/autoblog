...
export async function dedupeStep(state, config, options) {
  ...
  if (scheduleResult.skipDedupe) {
    ...
    // Track cost estimate for dedupe call
    if (options.costTracker) {
      options.costTracker.addGemini('dedupe', { promptTokenCount: 800, candidatesTokenCount: 200, totalTokenCount: 1000 }, config.models?.text || 'gemini-3-flash-preview');
    }
