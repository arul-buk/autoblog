...
const DEFAULTS = {
  models: {
    text: process.env.AUTOBLOG_TEXT_MODEL || 'gemini-3-flash-preview',
    image: process.env.AUTOBLOG_IMAGE_MODEL || 'gemini-3.1-flash-image-preview',
  },
  retry: {
    maxAttempts: 3,
    baseDelayMs: 2000,
  ...
  contentRefresh: {
    enabled: false,
    autoRefresh: false,    // when true, top stale post becomes next topic in default pipeline
    rules: [
  ...
  geoTracking: {
    enabled: false,
    brandNames: [],                // your brand names to check for in AI answers
    competitorBrands: [],          // competitor brand/domain strings to monitor
    monitorQueries: [],            // specific queries to always track (e.g., GSC top queries)
    maxKeywords: 20,               // cap per audit run to control API costs
  ...
  contentQuality: {
    // First-party data to inject into writer prompt (proprietary data, user insights, etc.)
    firstPartyData: null,
    // Minimum cross-model review score to publish (0 = always publish)
    minPublishScore: 0,
    // Maximum local content pages per location template
    maxLocalPagesPerTemplate: 5,
    // Randomize publishing cadence (jitter in hours added to cron schedule)
    cadenceJitterHours: 0,
    // Skip publishing if no original insight can be added
    requireOriginality: false,
  },
};
