 ...
 * Supports both service account keys and OAuth user credentials (authorized_user).
 */
function loadCredentials() {
  ...
  try {
    if (existsSync(value)) {
      parsed = JSON.parse(readFileSync(value, 'utf-8'));
    }
...
/**
 * Get access token from OAuth user credentials (authorized_user type).
 */
async function getAccessTokenFromOAuth(credentials) {
  const response = await fetch(TOKEN_URL, {
    method: 'POST',
    headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
  ...
  if (!response.ok) {
    throw new Error(`GSC OAuth token refresh failed (${response.status}): ${await response.text()}`);
  }
 ...
 */
async function getAccessTokenFromCredentials(credentials) {
  if (credentials.type === 'authorized_user') {
    return getAccessTokenFromOAuth(credentials);
  }
  // Service account (has client_email + private_key)
...
/**
 * Resolve frequency config to number of days.
 *
 * @param {string|number} frequency - Preset name or custom day count
 * @returns {number} Days between GSC runs
 */
function resolveFrequencyDays(frequency) {
  if (typeof frequency === 'number') return frequency;
  return FREQUENCY_DAYS[frequency] ?? FREQUENCY_DAYS['weekly'];
}

/**
 * Check whether GSC mining should run based on the configured schedule.
 * Returns { shouldRun: boolean, reason: string, daysSinceLastRun: number|null }.
 *
 * @param {object} config - Full autoblog config
 * @returns {{ shouldRun: boolean, reason: string, daysSinceLastRun: number|null }}
 */
export function shouldRunGsc(config) {
  const schedule = config.gsc?.schedule || {};
  const frequency = schedule.frequency ?? 'weekly';
  const intervalDays = resolveFrequencyDays(frequency);

  // 'every-run' always proceeds
  if (intervalDays === 0) {
    return { shouldRun: true, reason: 'frequency set to every-run', daysSinceLastRun: null };
  }

  const lastRunFile = path.resolve(process.cwd(), schedule.lastRunFile || '.autoblog-gsc-lastrun');

  if (!existsSync(lastRunFile)) {
    return { shouldRun: true, reason: 'no previous GSC run recorded', daysSinceLastRun: null };
  }

  try {
    const lastRunIso = readFileSync(lastRunFile, 'utf-8').trim();
    const lastRunDate = new Date(lastRunIso);
    if (isNaN(lastRunDate.getTime())) {
      return { shouldRun: true, reason: 'invalid last-run timestamp — re-running', daysSinceLastRun: null };
    }

    const daysSince = (Date.now() - lastRunDate.getTime()) / 86400000;
    if (daysSince >= intervalDays) {
      return { shouldRun: true, reason: `${Math.floor(daysSince)}d since last run (threshold: ${intervalDays}d)`, daysSinceLastRun: Math.floor(daysSince) };
    }

    return {
      shouldRun: false,
      reason: `last GSC run was ${Math.floor(daysSince)}d ago (next in ${Math.ceil(intervalDays - daysSince)}d)`,
      daysSinceLastRun: Math.floor(daysSince),
    };
  } catch {
    return { shouldRun: true, reason: 'could not read last-run file — re-running', daysSinceLastRun: null };
  }
}

/**
 * Record the current timestamp as the last GSC run.
 *
 * @param {object} config - Full autoblog config
 */
export function updateGscLastRun(config) {
  const schedule = config.gsc?.schedule || {};
  const lastRunFile = path.resolve(process.cwd(), schedule.lastRunFile || '.autoblog-gsc-lastrun');
  writeFileSync(lastRunFile, new Date().toISOString(), 'utf-8');
}
...
export async function mineGscTopics(config) {
  ...
  const quotaProject = config.gsc?.quotaProject || credentials.quota_project_id || null;

  const accessToken = await withRetry(
    () => getAccessTokenFromCredentials(credentials),
