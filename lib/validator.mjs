...
export function validatePost(content, config, options = {}) {
  ...
  if (options.existingSlugs) {
    const prefix = config.output?.contentPathPrefix || '/blog/';
    ...
    for (const linkPattern of linkPatterns) {
      ...
      while ((match = linkPattern.exec(content)) !== null) {
        ...
        if (!options.existingSlugs.includes(linkedSlug)) {
          warnings.push(`Internal link to non-existent post: ${prefix}${linkedSlug}`);
        }
      }
    }
  }
...
function checkGeoAeoCompliance(content, config) {
  ...
  // 1. TL;DR section present (optional bonus — not every post needs one)
  const hasTldr = bodyLower.includes('tl;dr') || bodyLower.includes('tldr') || body.includes('tldr-section');
  if (hasTldr) {
    scorePoints++;
  }
  // No warning — TL;DR is optional to avoid template footprint

  // 2. Key Takeaways section present (optional bonus)
  const hasTakeaways = bodyLower.includes('key takeaways') || bodyLower.includes('key-takeaways');
  if (hasTakeaways) {
    scorePoints++;
  }
  // No warning — Key Takeaways is optional to avoid template footprint

  // 3. FAQ section present (optional — not every post needs one)
  const hasFaq = bodyLower.includes('frequently asked questions') || bodyLower.includes('faq-section') || bodyLower.includes('faq');
  if (hasFaq) {
    scorePoints++;
  }
  // No warning if missing — FAQ is optional to avoid template footprint
