...
function buildAuditMessage(auditReport, geoMetrics, config) {
  ...
  if (geoMetrics) {
    ...
    if (geoMetrics.competitorPresence && Object.keys(geoMetrics.competitorPresence).length > 0) {
      lines.push('');
      lines.push('<b>Competitor presence in AI answers:</b>');
      for (const [brand, data] of Object.entries(geoMetrics.competitorPresence).slice(0, 5)) {
        lines.push(`  • ${escapeHtml(brand)}: ${data.mentionCount} queries (${(data.mentionRate * 100).toFixed(0)}%)`);
      }
    }
    if (geoMetrics.competitorOnlyKeywords?.length > 0) {
      lines.push(`  ⚠️ ${geoMetrics.competitorOnlyKeywords.length} queries where competitors appear but you don't`);
    }
  }
