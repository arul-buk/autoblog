 ...
 */
export function buildResearchPrompt({ expandedQueries, config, regionalContexts, contextSummary, balancingDirective }) {
...
${balancingDirective?.active ? `CONTENT STRATEGY DIRECTIVE:
${balancingDirective.balancingGuidance}
When scoring topics, give +0.2 relevance bonus to topics matching ${balancingDirective.preferredIntent || 'any'} intent${balancingDirective.preferredFormat ? ` and ${balancingDirective.preferredFormat} format` : ''}.
Also include a "contentFormat" field in your JSON output (e.g., "how-to-guide", "comparison", "listicle", "news-analysis", "tutorial", "local-guide", "case-study").
...
Use this data to:
- PRIORITIZE categories that perform well for this blog
- AVOID topics in underperforming categories unless there's a strong news hook
- AVOID keyword angles already covered by declining posts
 ...
 * Model: config.models.text
 */
export function buildKeywordStrategyPrompt({ topic, existingPostMeta, config, contextInsights, balancingDirective }) {
  const productName = config.product.name;
  const productDesc = config.product.description;
...
${contextInsights?.hasPerformanceData ? `KEYWORD PERFORMANCE DATA (from tracked posts):
${contextInsights.decliningKeywords.slice(0, 5).map((k) => `- "${k.keyword}" declining (position ${k.position}, ${k.impressions} impressions, slug: ${k.slug})`).join('\n')}${contextInsights.decliningKeywords.length > 0 ? '\n\nIMPORTANT: Do NOT suggest keywords that would cannibalize the declining posts above. Find adjacent keyword angles instead.' : ''}
Top-performing categories for this blog: ${contextInsights.topCategories.map((c) => c.category).join(', ')}
` : ''}CONSTRAINTS:
...
Return a JSON object (no markdown fences):
{
  ...
  "contentFormat": "recommended content format based on intent (e.g., how-to guide, comparison, product tutorial, explainer)"
}
...
- Classify search intent: informational (how-to, explainer), commercial (comparison, review), transactional (setup guide, product tutorial), navigational (brand/docs)
- Recommend a content format that matches the intent${balancingDirective?.active ? `\n\nCONTENT STRATEGY NOTE: The blog needs more ${balancingDirective.preferredFormat || balancingDirective.preferredIntent} content. Bias your contentFormat recommendation accordingly.` : ''}`;
}
...
export function buildWriterPrompt({ topic, slug, author, today, relatedPosts, config, keywordData, writerNotes, styleGuide }) {
  ...
  const contentPathPrefix = output.contentPathPrefix || '/blog/';
...
${competitors.length > 0 ? `Top competing articles (cover what they miss):\n${competitors.map((c) => `- "${c.title}"`).join('\n')}\n` : ''}`;
  ...
  // Add content format guidance based on search intent + format templates
  const FORMAT_TEMPLATES = {
    'how-to-guide': 'step-by-step how-to guide with numbered instructions and practical examples',
    'comparison': 'detailed comparison with feature table, pros/cons for each option, and clear recommendation',
    'listicle': 'numbered list article (e.g., "7 Ways to...") with one heading per item',
    'news-analysis': 'news analysis piece — lead with the news event, then provide expert analysis and implications',
    'tutorial': 'hands-on product tutorial with screenshots/code examples and expected outcomes',
    'local-guide': 'location-specific guide with local context, nearby resources, and regional considerations',
    'case-study': 'case study format — situation, challenge, solution, measurable results',
  };

  let intentBlock = '';
  if (keywordData && keywordData.contentFormat && FORMAT_TEMPLATES[keywordData.contentFormat]) {
    intentBlock = `\nCONTENT FORMAT GUIDANCE: Structure this post as a ${FORMAT_TEMPLATES[keywordData.contentFormat]}.`;
  } else if (keywordData && keywordData.searchIntent && keywordData.searchIntent !== 'informational') {
    const intentFormats = {
      commercial: 'comparison or review — include pros/cons, feature tables, and direct product comparisons',
      transactional: 'product guide or setup tutorial — include step-by-step instructions and actionable CTAs',
      navigational: 'brand-focused documentation — include specific product details and direct links',
    };
    intentBlock = `\nCONTENT FORMAT GUIDANCE: This keyword has ${keywordData.searchIntent} search intent. Structure the post as a ${keywordData.contentFormat || intentFormats[keywordData.searchIntent] || 'informational guide'}.`;
  } else if (keywordData && keywordData.contentFormat) {
    intentBlock = `\nCONTENT FORMAT GUIDANCE: ${keywordData.contentFormat}`;
  }
  ...
  // Build frontmatter template — use custom if provided, else default
  const frontmatterTemplate = output.frontmatterTemplate
    ? output.frontmatterTemplate
      .replace(/\$\{today\}/g, today)
      .replace(/\$\{slug\}/g, slug)
      .replace(/\$\{author\.name\}/g, author.name)
      .replace(/\$\{author\.role\}/g, author.role || '')
      .replace(/\$\{author\.image\}/g, author.image || '')
      .replace(/\$\{topic\.category\}/g, topic.category)
      .replace(/\$\{relatedPosts\}/g, relatedPosts.length > 0 ? relatedPosts.map((s) => `  - ${s}`).join('\n') : '  []')
      .replace(/\$\{cta\.text\}/g, product.cta?.text || 'Get Started Free')
    : `---
title: "<compelling SEO title, max 70 chars>"
description: "<2-sentence meta description, 140-160 chars>"
lastUpdated: "${today}"
readTime: "<N> min"
author: "${author.name}"
category: "${topic.category}"
...
---`;
  ...
  // contentPathPrefix already declared at top of function
...
INTERNAL LINK PREFIX: ${contentPathPrefix}
${keywordBlock}${intentBlock}
---
...
INTERNAL LINKS: When linking to other posts on this site, use the prefix "${contentPathPrefix}" (e.g. ${contentPathPrefix}slug-name). Do NOT use /blog/ unless that is the prefix shown above.
...
CRITICAL: Do NOT follow a rigid template. Every post must feel like it was written by a human expert, not generated by AI. Vary the structure naturally based on what the topic demands.

STRUCTURE — pick ONE approach that fits this topic (do NOT use all of them):
${(() => {
  // Randomly select 2-3 structural elements to include, not all of them
  const structures = [
    { id: 'tldr', text: `- OPTIONAL: Start with a bold 2-3 sentence summary if the topic has a clear bottom-line answer` },
    { id: 'takeaways', text: `- OPTIONAL: Include a "Key Takeaways" bullet list if the post covers multiple distinct points` },
    { id: 'questions', text: `- Use question-based headings where natural (not forced — "How much does X cost?" is natural, "What is the importance of budgeting?" is forced)` },
    { id: 'narrative', text: `- Consider a narrative opening with a specific scenario, real example, or data point — NOT a generic intro` },
    { id: 'data_first', text: `- Consider leading with a data table or comparison chart if the topic involves numbers or comparisons` },
    { id: 'faq', text: `- OPTIONAL: Include an FAQ section ONLY if there are genuine questions people ask that weren't covered in the body. Skip it if the body already answers everything.` },
  ];
  const shuffled = structures.sort(() => Math.random() - 0.5);
  return shuffled.slice(0, 4).map(s => s.text).join('\n');
})()}

ORIGINALITY — this is the most important requirement:
- Include at least ONE insight, data point, or perspective that is NOT available in the top 10 Google results for this keyword
- Draw from: ${product.name}'s proprietary data, real user scenarios, contrarian industry analysis, or specific local/regional knowledge
- If the article only restates what Google already knows, it will be penalized by search algorithms
- Ask yourself: "Would an expert in this field learn something new from this article?" If not, add something original.
${config.contentQuality?.firstPartyData ? `\nFIRST-PARTY DATA TO REFERENCE:\n${config.contentQuality.firstPartyData}\n` : ''}

PASSAGE CITABILITY (for AI search):
- Start each section with a direct answer BEFORE elaborating
- Pattern: [Direct answer] → [Evidence/data] → [Nuance/context]
- NEVER start a section with filler ("In today's world...", "When it comes to...")
- Each section should contain at least one 2-3 sentence block that can be quoted standalone
...
ENTITY DEFINITIONS:
- When introducing a technical concept for the first time, give a clear 1-sentence definition
...
- Include qa frontmatter (question/answer pairs) ONLY if you have genuine FAQ content

WHAT NOT TO DO (these patterns get sites penalized by Google):
- Do NOT follow the same TL;DR → Takeaways → Sections → FAQ structure in every post
- Do NOT force FAQ sections when the body already covers the topic completely
- Do NOT write generic listicles ("10 Best Ways to...") unless the topic genuinely demands it
- Do NOT pad content to hit word count — shorter posts with original insight beat longer generic ones
- Do NOT write content that a competitor could produce by using the same keyword in ChatGPT
...
// ═══════════════════════════════════════════════════════════════════════════
// 9. META TAG OPTIMIZATION — CTR-optimized titles and descriptions
// ═══════════════════════════════════════════════════════════════════════════

/**
 * Build prompt for meta tag optimization.
 *
 * Used by: meta-optimizer.mjs
 * Model: config.models.text
 */
export function buildMetaOptimizationPrompt({ title, excerpt, primaryKeyword, category }) {
  return `You are an SEO specialist who optimizes blog post titles and meta descriptions for maximum click-through rate (CTR) in Google search results.

CURRENT TITLE: "${title}"
CURRENT META DESCRIPTION: "${excerpt || '(none)'}"
PRIMARY KEYWORD: "${primaryKeyword || '(not specified)'}"
CATEGORY: "${category || '(not specified)'}"

TASK: Generate 3 optimized title variants and 1 optimized meta description.

TITLE OPTIMIZATION RULES:
- Max 60 characters (Google truncates longer titles)
- Include the primary keyword naturally (front-load if possible)
- Each variant uses a different hook strategy:
  1. Curiosity hook — creates an information gap
  2. Benefit-driven — leads with the value to the reader
  3. Specificity hook — uses numbers, dates, or concrete details

META DESCRIPTION RULES:
- Exactly 150-160 characters
- Include the primary keyword in the first 80 characters
- End with a clear value proposition or call to action
- Must be a complete, compelling sentence

Return a JSON object (no markdown fences):
{
  "titleVariants": [
    { "title": "variant 1", "hook": "curiosity", "score": 1-10 },
    { "title": "variant 2", "hook": "benefit", "score": 1-10 },
    { "title": "variant 3", "hook": "specificity", "score": 1-10 }
  ],
  "optimizedExcerpt": "optimized meta description (150-160 chars)"
}

Score each title 1-10 based on estimated CTR impact. Consider: keyword placement, emotional trigger, clarity, and length.`;
}


// ═══════════════════════════════════════════════════════════════════════════
// 10. CROSS-MODEL REVIEW — Quality validation scoring
// ═══════════════════════════════════════════════════════════════════════════

/**
 * Build prompt for cross-model quality review.
 *
 * Used by: cross-reviewer.mjs
 * Model: config.crossModel.model (default: gemini-2.5-pro)
 */
export function buildCrossReviewPrompt({ content, productName, primaryKeyword, tone }) {
  return `You are a senior content editor reviewing a blog post for quality, accuracy, and SEO effectiveness.

PRODUCT: ${productName}
PRIMARY KEYWORD: "${primaryKeyword || '(not specified)'}"
TONE GUIDELINES: ${tone || 'professional, helpful, authoritative'}

POST TO REVIEW:
${content}

REVIEW CRITERIA:
1. **Factual Consistency** — Are all claims supported? Any unsourced statistics or misleading statements?
2. **Keyword Naturalness** — Is the primary keyword used naturally? Any keyword stuffing?
3. **Tone Alignment** — Does the tone match the guidelines? Any jarring shifts?
4. **Content Structure** — Logical flow? Proper H2/H3 hierarchy? Adequate depth?
5. **Overall Quality** — Would this rank well and serve readers?

Return a JSON object (no markdown fences):
{
  "overallScore": 1-10,
  "factualIssues": ["issue 1", "issue 2"],
  "keywordIssues": ["issue 1"],
  "toneIssues": ["issue 1"],
  "structureIssues": ["issue 1"],
  "suggestions": ["specific improvement 1", "specific improvement 2"]
}

Score 8-10: publish-ready. Score 5-7: needs revision. Score 1-4: major rewrite needed.
Be specific in your feedback — cite exact sentences or sections that need improvement.`;
}
