...
- Recommend a content format that matches the intent`;
}
...
${competitors.length > 0 ? `Top competing articles (cover what they miss):\n${competitors.map((c) => `- "${c.title}"`).join('\n')}\n` : ''}`;
  ...
  // Add content format guidance based on search intent
  let intentBlock = '';
  if (keywordData && keywordData.searchIntent && keywordData.searchIntent !== 'informational') {
    const intentFormats = {
  ...
  if (keywordData && keywordData.contentFormat && !intentBlock) {
