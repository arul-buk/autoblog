 ...
 */
export async function getIntelligentSeeds(apiKey, config, topic, existingPostMeta = [], contextInsights = null, balancingDirective = null) {
  ...
  try {
    ...
    const prompt = buildKeywordStrategyPrompt({ topic, existingPostMeta, config, contextInsights, balancingDirective });
    ...
    try {
      ...
      // Try to extract JSON object from malformed/wrapped response
    ...
    const searchIntent = parsed.searchIntent || 'informational';
    const contentFormat = parsed.contentFormat || null;

    if (seeds.length === 0) {
      console.log('  Gemini returned no seeds — falling back to title extraction');
      return { seeds: fallbackSeeds(topic), questions: [], gapAnalysis: '', avoidKeywords: [], searchIntent: 'informational', contentFormat: null, skip: false };
    }
    ...
    if (searchIntent !== 'informational') console.log(`  Search intent: ${searchIntent}`);

    return { seeds, questions, gapAnalysis, avoidKeywords, searchIntent, contentFormat, skip: false };
  } catch (err) {
...
export async function researchKeywords(config, topic, options = {}) {
  let seeds;
  let seedSearchIntent = topic.searchIntent || 'informational';
  let seedContentFormat = topic.contentFormat || null;
  if (options.seedKeywords) {
    seeds = options.seedKeywords;
  } else if (options.apiKey) {
    const seedResult = await getIntelligentSeeds(options.apiKey, config, topic, options.existingPostMeta || [], options.contextInsights, options.balancingDirective);
    seeds = seedResult.seeds;
    seedSearchIntent = seedResult.searchIntent || seedSearchIntent;
    seedContentFormat = seedResult.contentFormat || seedContentFormat;
  } else {
  ...
  const result = {
    ...
    competitorAngles: [],
    questions: [],
    searchIntent: seedSearchIntent,
    contentFormat: seedContentFormat,
    skip: false,
  };
  ...
  try {
    ...
    const resultItems = overviewData?.tasks?.[0]?.result || [];
    // DataForSEO nests keyword items inside result[0].items
    const flatItems = resultItems.flatMap((r) => r.items || []);
    const keywords = flatItems
      ...
      .filter((k) => k.difficulty === null || k.difficulty <= maxDifficulty)
      .sort((a, b) => (b.volume || 0) - (a.volume || 0));

    if (keywords.length > 0) {
      result.primaryKeyword = keywords[0];
      const volStr = result.primaryKeyword.volume !== null ? result.primaryKeyword.volume : 'n/a';
      const diffStr = result.primaryKeyword.difficulty !== null ? result.primaryKeyword.difficulty : 'n/a';
      console.log(`  Primary keyword: "${result.primaryKeyword.keyword}" (vol: ${volStr}, diff: ${diffStr})`);
    }
  } catch (err) {
    ...
    try {
      ...
      result.secondaryKeywords = relatedItems
        .filter((item) => {
          ...
          const diff = info.keyword_difficulty;
          // Keep keywords with null volume (no data) or volume above threshold
          return (vol === null || vol >= minVolume) && (diff === null || diff <= maxDifficulty);
        })
        .map((item) => ({
  ...
  // If no primary keyword found from DataForSEO, fall back to first seed keyword
  if (!result.primaryKeyword) {
    if (seeds.length > 0) {
      result.primaryKeyword = { keyword: seeds[0], volume: null, difficulty: null };
      result.secondaryKeywords = seeds.slice(1).map((k) => ({ keyword: k, volume: null, difficulty: null }));
      console.log('  No DataForSEO data — using Gemini seed keywords');
    } else {
